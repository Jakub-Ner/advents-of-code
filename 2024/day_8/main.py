...
all_antinodes: dict[str,set] = {}
for antena_name, points in antenas.items():
    all_antinodes[antena_name] = set()
    for point in points:
        for other_point in points:
            if point != other_point:
                antinode = find_antinode(point, other_point)
                if antinode:
                    all_antinodes[antena_name].add(antinode)


merged = set()
for antena_name, antinodes in all_antinodes.items():
    merged = merged.union(antinodes)
